...
namespace ns3
{
    ...
    Ptr<Node> PointToPointDumbbellHelper::GetLeft() const { // Get the left side bottleneck router
        return m_routers.Get(0);
    }

    Ptr<Node> PointToPointDumbbellHelper::GetLeft(uint32_t i) const { // Get the i'th left side leaf
        return m_leftLeaf.Get(i);
    }

    Ptr<Node> PointToPointDumbbellHelper::GetRight() const { // Get the right side bottleneck router
        return m_routers.Get(1);
    }

    Ptr<Node> PointToPointDumbbellHelper::GetRight(uint32_t i) const { // Get the i'th right side leaf
    ...
    uint32_t PointToPointDumbbellHelper::LeftCount() const { // Number of left side nodes
        return m_leftLeaf.GetN();
    }

    uint32_t PointToPointDumbbellHelper::RightCount() const { // Number of right side nodes
