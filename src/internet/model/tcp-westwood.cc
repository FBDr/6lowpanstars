...
namespace ns3
{
    ...
    void
    TcpWestwood::NewAck(const SequenceNumber32 & seq) { // Same as Reno
        ...
        if (m_inFastRec) {// First new ACK after fast recovery, reset cwnd as in Reno
            m_cWnd = m_ssThresh;
            m_inFastRec = false;
        ...
        if (m_cWnd < m_ssThresh) { // Slow start mode, add one segSize to cWnd as in Reno
            m_cWnd += m_segmentSize;
            NS_LOG_INFO("In SlowStart, updated to cwnd " << m_cWnd << " ssthresh " << m_ssThresh);
        } else { // Congestion avoidance mode, increase by (segSize*segSize)/cwnd as in Reno
    ...
    TcpWestwood::ReceivedAck(Ptr<Packet> packet, const TcpHeader & tcpHeader) {
        NS_LOG_FUNCTION(this);
        int acked = 0;
        if ((0 != (tcpHeader.GetFlags() & TcpHeader::ACK)) && tcpHeader.GetAckNumber() >= m_prevAckNo) {// It is a duplicate ACK or a new ACK. Old ACK is ignored.
            if (m_pType == TcpWestwood::WESTWOOD) {// For Westwood, calculate the number of ACKed segments and estimate the BW
                acked = CountAck(tcpHeader);
                EstimateBW(acked, tcpHeader, Time(0));
            } else if (m_pType == TcpWestwood::WESTWOODPLUS) {// For Weswood+, calculate the number of ACKed segments and update m_ackedSegments
                if (m_IsCount) {
                    acked = CountAck(tcpHeader);
    ...
    TcpWestwood::CountAck(const TcpHeader & tcpHeader) {
        ...
        if (cumul_ack == 0) {// A DUPACK counts for 1 segment delivered successfully
            m_accountedFor++;
            cumul_ack = 1;
        }
        if (cumul_ack > 1) {// A delayed ACK or a cumulative ACK after a retransmission
    ...
    TcpWestwood::DupAck(const TcpHeader& header, uint32_t count) {
        ...
        if (count == 3 && !m_inFastRec) {// Triple duplicate ACK triggers fast retransmit
            ...
            DoRetransmit();
        } else if (m_inFastRec) {// Increase cwnd for every additional DUPACK as in Reno
            m_cWnd += m_segmentSize;
