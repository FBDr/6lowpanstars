...
namespace ns3
{
    ...
    Ptr<Packet>
        ...
        if (m_data.size() == 0) { // No actual data, just return dummy-data packet of correct size
        ...
        for (BufIterator i = m_data.begin(); i != m_data.end(); ++i) {
            pktCount++;
            pktSize = (*i)->GetSize();
            if (!beginFound) { // Look for first fragment
                if (count + pktSize > offset) {
                    ...
                    if (fragmentLength >= s) { // Data to be copied falls entirely in this packet
                        return (*i)->CreateFragment(packetOffset, s);
                    } else { // This packet only fulfills part of the request
                        outPacket = (*i)->CreateFragment(packetOffset, fragmentLength);
                    }
                    NS_LOG_LOGIC("Output packet is now of size " << outPacket->GetSize());
                }
            } else if (count + pktSize >= offset + s) { // Last packet fragment found
    ...
    TcpTxBuffer::DiscardUpTo(const SequenceNumber32 & seq) {
        ...
        while (i != m_data.end()) {
            if (offset > (*i)->GetSize()) { // This packet is behind the seqnum. Remove this packet from the buffer
                pktSize = (*i)->GetSize();
                m_size -= pktSize;
                offset -= pktSize;
            ...
            } else if (offset > 0) { // Part of the packet is behind the seqnum. Fragment
