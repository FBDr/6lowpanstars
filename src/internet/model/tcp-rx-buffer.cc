...
namespace ns3
{
    ...
    TcpRxBuffer::MaxRxSequence(void) const {
        if (m_gotFin) { // No data allowed beyond FIN
            return m_finSeq;
        } else if (m_data.size()) { // No data allowed beyond Rx window allowed
    ...
    TcpRxBuffer::Add(Ptr<Packet> p, TcpHeader const& tcph) {
        ...
        while (i != m_data.end() && i->first <= tailSeq) {
            ...
            if (lastByteSeq > headSeq) {
                if (i->first > headSeq && lastByteSeq < tailSeq) { // Rare case: Existing packet is embedded fully in the new packet
