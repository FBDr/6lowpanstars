...
namespace ns3
{
    ...
    NscTcpSocketImpl::Close(void) {
        ...
        if (!m_txBuffer.empty()) { // App close with pending data must wait until all data transmitted
            m_closeOnEmpty = true;
    ...
    void NscTcpSocketImpl::ConnectionSucceeded() { // We would preferred to have scheduled an event directly to
    ...
    bool NscTcpSocketImpl::Accept(void) {
        if (m_state == CLOSED) { // Happens if application closes listening socket after Accept() was scheduled.
            return false;
        }
        NS_ASSERT(m_state == LISTEN);
