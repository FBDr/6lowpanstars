...
namespace ns3
{
            ...
            m_timestampToEcho(sock.m_timestampToEcho)
 {
    ...
    TcpSocketBase::DoConnect(void) {
        ...
        if (m_state == CLOSED || m_state == LISTEN || m_state == SYN_SENT || m_state == LAST_ACK || m_state == CLOSE_WAIT) { // send a SYN packet and change state into SYN_SENT
    ...
    TcpSocketBase::DoForwardUp(Ptr<Packet> packet, const Address &fromAddress,
        ...
        if (m_rWnd.Get() == 0 && tcpHeader.GetWindowSize() != 0 && m_persistEvent.IsRunning()) { // persist probes end
    ...
    TcpSocketBase::ProcessSynSent(Ptr<Packet> packet, const TcpHeader & tcpHeader) {
        ...
        if (tcpflags == 0) { // Bare data, accept it and move to ESTABLISHED state. This is not a normal behaviour. Remove this?
            ...
            SendEmptyPacket(TcpHeader::SYN | TcpHeader::ACK);
                ...
                && m_nextTxSequence + SequenceNumber32(1) == tcpHeader.GetAckNumber()) { // Handshake completed
            NS_LOG_INFO("SYN_SENT -> ESTABLISHED");
            m_state = ESTABLISHED;
            m_connected = true;
            m_retxEvent.Cancel();
    ...
    TcpSocketBase::ProcessSynRcvd(Ptr<Packet> packet, const TcpHeader& tcpHeader,
        ...
        if (tcpflags == 0
                ...
                && m_nextTxSequence + SequenceNumber32(1) == tcpHeader.GetAckNumber())) { // If it is bare data, accept it and move to ESTABLISHED state. This is
    ...
    TcpSocketBase::ProcessWait(Ptr<Packet> packet, const TcpHeader & tcpHeader) {
        ...
        if (packet->GetSize() > 0 && tcpflags != TcpHeader::ACK) { // Bare data, accept it
            ...
            if (m_state == FIN_WAIT_1 && m_txBuffer->Size() == 0
                    && tcpHeader.GetAckNumber() == m_highTxMark + SequenceNumber32(1)) { // This ACK corresponds to the FIN sent
    ...
    TcpSocketBase::SendDataPacket(SequenceNumber32 seq, uint32_t maxSize, bool withAck) {
        ...
        if (isRetransmission == false) { // This is the next expected one, just log at end
    ...
    TcpSocketBase::ReceivedData(Ptr<Packet> p, const TcpHeader & tcpHeader) {
        ...
        if (m_rxBuffer->Size() > m_rxBuffer->Available() || m_rxBuffer->NextRxSequence() > expectedSeq + p->GetSize()) { // A gap exists in the buffer, or we filled a gap: Always ACK
            SendEmptyPacket(TcpHeader::ACK);
