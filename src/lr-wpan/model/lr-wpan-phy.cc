...
namespace ns3 {
    ...
    LrWpanPhy::ppduHeaderSymbolNumbers[7] = {
        { 32.0, 8.0, 8.0},
        { 32.0, 8.0, 8.0},
        { 2.0, 1.0, 0.4},
        { 6.0, 1.0, 1.6},
    ...
    LrWpanPhy::StartRx(Ptr<SpectrumSignalParameters> spectrumRxParams) {
        ...
        if (m_trxState == IEEE_802_15_4_PHY_RX_ON && !m_setTRXState.IsRunning()) {
            ...
            *interferenceAndNoise += *m_noise;
            ...
            NS_LOG_FUNCTION(this << " transceiver not in RX state");
            m_phyRxDropTrace(p);
    ...
    LrWpanPhy::EndTx(void) {
        NS_LOG_FUNCTION(this);
        //std::cout<<"!@@!@!@  Current state is "<<std::hex<< m_trxState<<std::endl;
