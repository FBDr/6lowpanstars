    ...
    namespace face {
        namespace tests {
            ...
            BOOST_AUTO_TEST_CASE(LinkServiceSendReceive) {
                ...
                face1->afterReceiveInterest.connect(bind([&nReceivedInterests] {
                    ++nReceivedInterests; }));
                face1->afterReceiveData.connect(bind([&nReceivedData] {
                    ++nReceivedData; }));
                face1->afterReceiveNack.connect(bind([&nReceivedNacks] {
                    ++nReceivedNacks; }));
