...
namespace nfd {
    namespace tests {
        ...
        BOOST_AUTO_TEST_CASE(AddRemove) {
            ...
            faceTable.afterAdd.connect([&] (const Face & face) {
                addHistory.push_back(face.getId()); });
            faceTable.beforeRemove.connect([&] (const Face & face) {
                removeHistory.push_back(face.getId()); });
