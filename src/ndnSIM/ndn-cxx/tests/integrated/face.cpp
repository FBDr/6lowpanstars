...
namespace ndn {
    namespace tests {
        ...
        {
            Face face;
            face.expressInterest(Name("/Hello/World/!"), bind([] {
            }), bind([] {
            }));
            BOOST_REQUIRE_NO_THROW(face.processEvents(time::seconds(1)));

            face.expressInterest(Name("/Bye/World/1"), bind([] {
            }), bind([] {
            }));
            face.expressInterest(Name("/Bye/World/2"), bind([] {
            }), bind([] {
            }));
            face.expressInterest(Name("/Bye/World/3"), bind([] {
            }), bind([] {
            }));
            face.shutdown();
