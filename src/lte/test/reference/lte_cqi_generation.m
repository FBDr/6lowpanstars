...
% % LTE from theory to practice
        ...
        fdl = 2120e6;
% DL carrier freq Hz, EARFCN = 100
        ful = 1930e6;
% UL carrier freq Hz, EARFCN = 18100

        nrbsControl = 25;
% tx bandwidth configuration in number of RBs
bwControl = nrbsControl * 180000;
% bandwidth in Hz, note that this is smaller than
        % the nominal Channel Bandwdith, see TS 36.101 fig 5.6 - 1

        nrbsData = 12;
% tx bandwidth configuration in number of RBs
bwData = nrbsData * 180000;
% bandwidth in Hz, note that this is smaller than
        % the nominal Channel Bandwdith, see TS 36.101 fig 5.6 - 1

        kT = -174;
% noise PSD in dBm / Hz
ndBmData = kT + 10 * log10(bwData);
% noise power dBm
ndBmControl = kT + 10 * log10(bwControl);
% noise power dBm
...
% % tx pow in W in DL
dlnf = 9;
% receiver noise figure in dB in DL
dlnControl = 10.^((ndBmControl - 30 + dlnf) / 10);
% % noise in W in DL
dlnData = 10.^((ndBmData - 30 + dlnf) / 10);
% % noise in W in DL
...
% % tx pow in W in UL
ulnf = 5;
% receiver noise figure in dB in UL
uln = 10.^((ndBmControl - 30 + ulnf) / 10);
% % noise in W in UL
...
privateRbUlsinr = ulp*g11ul / (ulp*g21ul + uln);
% % new CQI generation approach is only for DL, so in UL CQI is the same
