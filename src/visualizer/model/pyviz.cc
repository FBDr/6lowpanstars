...
namespace ns3{
    ...
    Mac48Address const &destinationAddress)
    {
        ...
        TxRecordValue record =
        { Simulator::Now(), node, false};
    ...
    PyViz::TraceNetDevRxCommon(std::string const &context, Ptr<const Packet> packet, Mac48Address const &from)
    {
        ...
        TransmissionSampleKey key =
        { record.srcNode, node, channel};
...
public:
    ...
    {
        FastClipping::Vector2 clipMin =
        { boundsX1, boundsY1}, clipMax =
        { boundsX2, boundsY2};
        FastClipping::Line line =
        {
            { lineX1, lineY1},
            { lineX2, lineY2}, (lineX2 - lineX1), (lineY2 - lineY1)
        };
